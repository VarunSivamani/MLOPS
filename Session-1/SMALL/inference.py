...
def main(model: str, image: str):
    ...
    with torch.no_grad():
        ...
        predicted_class = labels2class[str(classes.item())]
        predicted_confidence = conf.item()

    print(
        json.dumps({"predicted": predicted_class, "confidence": predicted_confidence})
    )
